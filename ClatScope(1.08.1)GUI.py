...
CLIENT_ID = "INSERT GOOGLE CUSTOM SEARCH CLIENT ID HERE"
HIBP_API_KEY = "INSERT HAVE I BEEN PWNED API KEY HERE"
HUNTER_API_KEY = "INSERT HUNTER API KEY HERE"
CASTRICK_API_KEY = "INSERT CASTRICKCLUES API KEY HERE"
VIRUSTOTAL_API_KEY = "INSERT VIRUSTOTAL API KEY HERE"
OPENAI_API_KEY = "INSERT OPENAI API KEY HERE"
PERPLEXITY_API_KEY = "INSERT PERPLEXITYY API KEY HERE" 
RAPIDAPI_KEY = "INSERT RAPIDAPI KEY HERE"
...
class ClatScopeGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("ClatScope Info Tool v1.08.1")
        ...
        # Track how many tasks are in progress to manage the "in progress" label
        self.task_count = 0
        ...
        ascii_art = (
            ...
            "╚██████╗    ███████╗    ██║  ██║       ██║       ███████║    ╚██████╗    ╚██████╔╝    ██║         ███████╗\n"
            " ╚═════╝    ╚══════╝    ╚═╝  ╚═╝       ╚═╝       ╚══════╝     ╚═════╝     ╚═════╝     ╚═╝         ╚══════╝\n"
            "C L A T S C O P E       I N F O       T O O L   (Version 1.08.1)\n"
        )
        ...
        # A label for showing "Your request is in progress..."
    ...
    def show_in_progress(self):
        """Increment task counter and set the progress label to show the 'in progress' message."""
        self.task_count += 1
        self.progress_label.config(text="Your request is in progress....")

    def hide_in_progress(self, future):
        """Decrement task counter when a submitted background task finishes. Hide message if none remain."""
        def _hide():
            self.task_count -= 1
    ...
    def create_buttons(self):
        functions = [
            ...
            ("36", "Domain Report (VirusTotal)", self.virustotal_domain_report_gui),  # Added button for VirusTotal
    ...
    # ----------------------  GUI Handlers for each function  ----------------------
    ...
    def ip_info(self, ip):
        try:
            ...
            self.gui_print(f"[!] > Error: {str(e)}")
            messagebox.showerror("IP Address Search", f"An error occurred: {str(e)}")
    ...
    def person_search(self, first_name, last_name, city):
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        " or most recent known activities. Each piece of information should be attributed to at least one credible source."
                    )
                },
                {
                    "role": "user",
        ...
        PERPLEXITY_API_KEY = "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"
        perplexity_headers = {
...
"""
    ...
    def email_lookup(self, email_address):
        try:
            ...
            msg = f"[!] > Invalid email address format: {str(e)}"
            self.gui_print(msg)
    ...
    def haveibeenpwned_check(self, email):
        ...
        try:
            ...
            if resp.status_code == 200:
                breaches = resp.json()
                box_width = 80
                header = f"Have I Been Pwned? - Breach Report for {email}"
                separator = "=" * box_width
                top_border = f"╭─{'─' * (box_width - 4)}─╮"
                header_line = f"| {header.center(box_width - 4)} |"
                ...
                for index, breach in enumerate(breaches, start=1):
                    ...
                    domain = breach.get('Domain', 'Unknown')[:60]
...
"""
            ...
            self.gui_print(msg)
            messagebox.showerror("Have I Been Pwned?", f"An error occurred: {str(e)}")
...
"""
            ...
            msg = f"[!] > Error: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("WHOIS Search", f"An error occurred: {str(e)}")
    ...
    def reverse_phone_lookup(self, phone_number):
        self.gui_print(f"Reverse phone lookup for {phone_number} not implemented.")
        messagebox.showinfo("Reverse Phone Search", "Reverse phone lookup not implemented.")
...
"""
            ...
            msg = f"[!] > An error occurred retrieving SSL cert info: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("SSL Search", msg)
...
"""
            ...
            msg = f"[!] > Error fetching metadata: {str(e)}"
            self.gui_print(msg)
...
Include analysis on political stability, crime, natural disaster risk, health risks, local laws, transportation, communication, travel advisories, required equipment, entry requirements, vaccination needs, currency restrictions, insurance needs, accommodation security, local transportation options, emergency contacts, embassy details, and more. Your analysis must include at least 350 words per section for general details, best practices, practical guidelines, and disclaimers. Answer all parameters fully.
"""
            ...
            msg = f"[!] > An error occurred: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("Travel Risk Assessment", f"An error occurred: {str(e)}")
    ...
    def botometer_search(self, username):
        try:
            ...
            msg = f"[!] > Error: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("Botometer Search", f"An error occurred: {str(e)}")

    def business_search(self, business_name):
        PERPLEXITY_API_URL = "https://api.perplexity.ai/chat/completions"
        PERPLEXITY_API_KEY = "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"
            ...
            "messages": [
                {
                    ...
                    "content":(
                        ...
                        "or business owner. Provide thorough, well-structured answers."
                    )
                },
                {
                    "role": "user",
        ...
        try:
            ...
            self.gui_print(out_text)
            messagebox.showerror("Business Search", f"An error occurred: {str(e)}")
    ...
    def hudson_rock_email_infection_check(self, email):
        try:
            ...
            msg = f"[!] > Error: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("HR Email Search", f"An error occurred: {str(e)}")
    ...
    def hudson_rock_username_infection_check(self, username):
        try:
            ...
            msg = f"[!] > Error: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("HR Username Search", f"An error occurred: {str(e)}")
    ...
    def hudson_rock_domain_infection_check(self, domain):
        try:
            ...
            msg = f"[!] > Error: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("HR Domain Search", f"An error occurred: {str(e)}")
    ...
    def hudson_rock_ip_infection_check(self, ip_address):
        try:
            ...
            msg = f"[!] > Error: {str(e)}"
            self.gui_print(msg)
            messagebox.showerror("HR IP Search", f"An error occurred: {str(e)}")
    ...
    def fact_check_text(self, text_to_check):
            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        "You are an advanced AI fact-checking assistant designed to evaluate claims and statements with rigorous accuracy. "
                        "Cite all sources at the end of the output."
                    )
                },
        ...
        try:
            ...
            self.gui_print(output_text)
            messagebox.showerror("Fact Check", f"An error occurred: {str(e)}")
        self.log_option(output_text)
    ...
    def relationship_search(self, query):
            ...
            "messages": [
                {
                    ...
                    "content": (
                        ...
                        "Include detailed citations for every claim."
                    )
                },
        ...
        try:
            ...
            self.gui_print(output_text)
            messagebox.showerror("Relationship Search", f"An error occurred: {str(e)}")
        self.log_option(output_text)

    def read_file_metadata(self, file_path):
        self.gui_print("File Metadata")
        try:
...
"""
    ...
    def hunter_domain_search(self, domain):
        try:
            ...
            self.gui_print(output_text)
            messagebox.showerror("Hunter.io Domain Search", f"An error occurred: {str(e)}")
            self.log_option(output_text)
    ...
    def hunter_email_finder(self, domain, first_name, last_name):
        try:
            ...
            output_text = f"[!] > Error: {str(e)}"
            self.gui_print(output_text)
            messagebox.showerror("Hunter.io Email Finder", f"An error occurred: {str(e)}")
            self.log_option(output_text)
    ...
    def hunter_email_verifier(self, email):
        try:
            ...
            output_text = f"[!] > Error: {str(e)}"
            self.gui_print(output_text)
            messagebox.showerror("Hunter.io Email Verification", f"An error occurred: {str(e)}")
            self.log_option(output_text)
    ...
    def hunter_company_enrichment(self, domain):
        try:
            ...
            output_text = f"[!] > Error: {str(e)}"
            self.gui_print(output_text)
            messagebox.showerror("Hunter.io Company Enrichment", f"An error occurred: {str(e)}")
            self.log_option(output_text)
    ...
    def hunter_person_enrichment(self, email):
        try:
            ...
            output_text = f"[!] > Error: {str(e)}"
            self.gui_print(output_text)
            messagebox.showerror("Hunter.io Person Enrichment", f"An error occurred: {str(e)}")
            self.log_option(output_text)
    ...
    def hunter_combined_enrichment(self, email):
        try:
            ...
            output_text = f"[!] > Error: {str(e)}"
            self.gui_print(output_text)
            messagebox.showerror("Hunter.io Combined Enrichment", f"An error occurred: {str(e)}")
            self.log_option(output_text)
    ...
    def castrick_email_search(self, email):
        ...
        try:
            ...
            output_text = f"[!] > Error: {str(e)}"
            self.gui_print(output_text)
            messagebox.showerror("CastrickClues Email Search", f"An error occurred: {str(e)}")
            self.log_option(output_text)

    # NEW: VirusTotal integration function
    def virustotal_domain_report(self, domain):
        ...
        try:
            ...
            self.gui_print(f"[!] > Exception: {str(e)}")
